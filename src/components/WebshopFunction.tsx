...
function WebshopFunction(props: Props) {
  ...
  useEffect(() => {
    axios.get("/webshop.json").then((response) => {
  ...
  const addToCart = (product: ProductProps, quantity: number) => {
    ...
    console.log("Tillagd produkt:", quantity, product.name);
    setAddedMessage(
      `${quantity}st '${product.name}' har lagts till i kundvagnen.`
    );
  };
        ...
        <>
          <StyledProductContainer>
            <StyledProductCards>
              {result.map((product) => (
                <Li key={product.id}>
                  <img style={imgStyle} alt="product" src={product.image} />
                  <p>{product.name}</p>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (quantityInput instanceof HTMLInputElement) {
                          addToCart(product, Number(quantityInput.value));
                        }
                      }}
                      type="button"
                    >
